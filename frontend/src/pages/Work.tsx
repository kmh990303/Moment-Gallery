...
const Work: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/works/edit/${art.id}`,
        {
          method: "PATCH",
  ...
  const handleDelete = async () => {
    const confirmed = window.confirm(
      "Are you really sure to delete this work?"
    );
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/works/${art.id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const isOwner = isLoggedIn && userId === art.userId;
      ...
      <header className="fixed top-0 left-0 w-full flex justify-between items-center p-4 text-white z-20 bg-black bg-opacity-75">
        ...
        <nav className="mr-8">
          <ul className="flex gap-8 text-xl">
            ...
            <li>
              <Link to="/work" className="hover:text-gray-400">
                Work
              </Link>
            </li>
            {!isLoggedIn && (
            ...
            {isLoggedIn && (
              <>
                ...
                <Link to="/edit" className="hover:text-gray-400">
                  Create
                </Link>
              </>
            )}
          ...
          {!IsEdit ? (
            <div className="flex gap-8 mt-8 justify-center">
              <div className="w-2/5 ml-8 translate-y-20">
                <img
                  src={`http://localhost:5000/${art.imagePath}`}
                  alt="art-image"
            ...
            <div className="flex gap-8 mt-8 justify-center">
              ...
              <form
                className="w-2/5 flex flex-col ml-8 translate-y-20"
